...
def detect(vehicle_list):
    # TODO: assuming that vehicle_list = [veh1, veh2] - can fix by checking for impact point definition
    vehicle_list[0].veh_model.head()
    # current location of impact point
    cgx1 = vehicle_list[0].veh_model.Dx.iloc[-1]  # take last value in Dx
    cgy1 = vehicle_list[0].veh_model.Dy.iloc[-1]  # take last value in Dy
    theta_rad1 = vehicle_list[0].veh_model.theta_rad.iloc[-1]
    ...
    # impact plane
    cgx2 = vehicle_list[1].veh_model.Dx.iloc[-1]
    cgy2 = vehicle_list[1].veh_model.Dy.iloc[-1]
    theta_rad2 = vehicle_list[1].veh_model.theta_rad.iloc[-1]
    ...
    edgeimpact_x1 = cgx2 + vehicle_list[1].edgeimpact_x1 * math.cos(theta_rad1) - vehicle_list[1].edgeimpact_y1 * math.sin(theta_rad1)
    edgeimpact_y1 = cgy2 + vehicle_list[1].edgeimpact_x1 * math.sin(theta_rad1) + vehicle_list[1].edgeimpact_y1 * math.cos(theta_rad1)
    edgeimpact_x2 = cgx2 + vehicle_list[1].edgeimpact_x2 * math.cos(theta_rad1) - vehicle_list[1].edgeimpact_y2 * math.sin(theta_rad1)
    edgeimpact_y2 = cgy2 + vehicle_list[1].edgeimpact_x2 * math.sin(theta_rad1) + vehicle_list[1].edgeimpact_y2 * math.cos(theta_rad1)
