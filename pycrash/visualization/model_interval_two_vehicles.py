...
def plot_motion_interval(veh_list, impactIndex, num_itter=10, imageDict=False, tire_path=True, show_vector=False):
    ...
    for veh in veh_list:
        ...
        for i in counter:
            ...
            if veh.striking and plotPOI:
                ...
                fig.add_trace(
                    go.Scatter(x=[veh.p_gx[f'POI_{plotImpactPoint}'][i], veh.p_gx[f'POI_{plotImpactPoint}'][i]],
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=False, title='x-axis - Forward (ft)', range=[1.5 * dx_min, 1.5 * dx_max],
                   constrain="domain"),
