...
def process_loadcell_asii(test_num, path, row_list, num_columns, impact_speed, english=True):
    """
    # this version differs from ver2 by adding a 0 before the test numbers < 100
    this version using the keyword "INTERFACE" to locate load cell data within the .EV5 file
    if you file does not have this, try ver1
    ...
    with open(os.path.join(path, format_file)) as myFile:
        for num, line in enumerate(myFile, 1):
            if 'INSTRUMENTATION' in line:
                ...
                print(f'Instrumentation data ends at line: {num}')
                instEnd = num
    ...
    with open(os.path.join(path, format_file)) as myFile:
        ...
        for num, line in enumerate(myFile, 1):
            ...
            if (num > instStart) & (num < instEnd) & ('INTERFACE' in line) & ('LC' in line):
                print(f'< Found LC Line at {num} >')
                print(line.split('|'))
                channel_name = line.split('|')[4]
                channel_num = int(line.split('|')[1])
                lc_row = channel_name[2]
                lc_col = int(channel_name[3])
            ...
            #accel_keywords 'AC', 'XG', 'V1 LEFT REAR', 'V1 RIGHT REAR'
            # prior search  elif (num > instStart) & (num < instEnd) & ('IC SENSOR' in line) & ('XG' in line) & (('DSLR' in line) | ('DSRR' in line)):
            elif (num > instStart) & (num < instEnd) & ('AC' in line) & ('XG' in line) & (('V1 LEFT REAR' in line) | ('V1 RIGHT REAR' in line)):
                print(f'< Found Acceleration Line at {num} >')
                channel_name = line.split('|')[4]
                channel_num = int(line.split('|')[1])
    ...
    for key, value in test_data.items():
        if value['type'] == 'LC':
            if (value['row'] in row_list) & ('No valid data' in key):
                ...
                print(f'loading LC file {value["fileName"]}')
            ...
            print(f'loading Accel file {value["fileName"]}')
            data_array = np.loadtxt(os.path.join(path, value["fileName"]), delimiter='\t')
            all_time = list(data_array[:, 0])
            all_data = list(data_array[:, 1])
            zero_time_index = all_time.index(0)
    ...
    # create dictionary of filtered force - only for selected rows
    forceDict = {}
    ...
    for key, value in test_data.items():
        ...
        if value['type'] == 'LC':
            if value['row'] == currentRow:
                ...
                rowSum = [0] * len(forceDict[next(iter(forceDict))]['time'])
                rowSum = [a + b for a, b in zip(rowSum, value['force'])]
                rowForce[value['row']] = rowSum
